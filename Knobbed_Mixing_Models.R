##### mixing models for knobbed whelk #####
# first run of mixing models for knobbed whelk
...
# subset for only knobbed whelk
whelk_data = whelk_data %>%
  filter(ProjID == "Whelk" & CommonName == "Knobbed Whelk")
  
# add in a k means column
whelk_data$kmeans = "Knobbed Whelk"

# put knobbed whelk in the dataframe
...
for (i in 1:length(n_tdf)) {
  ...
  model_fits$DIC[model_fits$tdf == n_tdf[i]] = simmr.out$output$`Knobbed Whelk`$BUGSoutput$DIC
