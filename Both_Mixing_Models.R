...
# first run of mixing models for both whelks together to choose a TDF
...
whelk_data = whelk_data %>%
  filter(ProjID == "Whelk" & CommonName %in% c("Smooth Whelk", "Knobbed Whelk"))

# add in a k means column
whelk_data$kmeans = "Whelk"
...
model_fits_smooth = data_frame(tdf = n_tdf, DIC = NA)
model_fits_knobbed = data_frame(tdf = n_tdf, DIC = NA)
...
for (i in 1:length(n_tdf)) {
  ...
  model_fits_smooth$DIC[model_fits$tdf == n_tdf[i]] = simmr.out$output$`Smooth Whelk`$BUGSoutput$DIC
  model_fits_knobbed$DIC[model_fits$tdf == n_tdf[i]] = simmr.out$output$`Knobbed Whelk`$BUGSoutput$DIC
